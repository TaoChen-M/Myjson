...
/*枚举中LEPT_FALSE值是1，如果返回v.type判断得话，这里应该是判断等于1*/
#define EXPECT_TRUE(actual) EXPECT_EQ_BASE((actual) != 1, "true", "false", "%s")
...
static void test_access_number() {
    ...
    lept_value v;
    lept_init(&v);
    lept_set_number(&v,10.33);
    EXPECT_EQ_DOUBLE(10.33, lept_get_number(&v));
    lept_free(&v);
}
